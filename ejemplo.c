...
void handle_sigalrm(int sig)
{
  ...
  for (int i = 0; i < len_numbers; i++)
  {
    ...
    send_signal_with_int(other_id, numbers[i]); // Le envía un número a cada segundo
...
int main(int argc, const char **argv)
{
  ...
  other_id = fork();
  printf(other_id);
  if (!other_id) // Solo el hijo cumple el if. La ejecución del hijo
  {
    // Conectamos SIGINT a handle_sgint y SIGUSR1 a handle_sigusr1
    signal(SIGINT, handle_sigint); // Escribe en un archivo los números que están en un arreglo
    connect_sigaction(SIGUSR1, handle_sigusr1); // Recibe el número que le envía el padre, printea y lo guarda en el arreglo

    // Se espera infinitamente
    while (true) // busywaiting: no muy recomendable
      ;
  }
  ...
  signal(SIGALRM, handle_sigalrm); // La ejecución del padre

  // Puedes intentar de cambiar la siguiente linea por alarm(1);
  kill(getpid(), SIGALRM);  // Al mismo proceso
